export const DEV_API_BASE_URL = "http://localhost:5154/api/1.0";
export const PROD_API_BASE_URL = "https://api.legallighthouse.xyz/api/1.0";

export const DEV_WEBSITE_URL = "http://localhost:3000";
export const PROD_WEBSITE_URL = "https://legallighthouse.xyz";

export const API_COOKIE_NAME = "Atlas.Identity";

export const DEV_SUPPORT_EMAIL = "support@legallighthouse.xyz";
export const PROD_SUPPORT_EMAIL = "support@legallighthouse.xyz";

export const TEXT_COLOUR = "#313131";
export const PRIMARY_COLOUR = "#8a50c4";
export const SECONDARY_COLOUR = "#EFEFEF";

export const GOOGLE_TRACKING_ID = "";
